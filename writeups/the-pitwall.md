...
This writeup focuses on how the data moves, from the game's memory, through the Electron client, across a WebSocket, and into a React UI updating in real time.
...
Viewer pages (both the full dashboard and the OBS overlays) follow the same connection pattern. On mount, the page gets `/api/socket` to ensure the Socket.io server is running, then subscribes to the channel's event stream:
